        ...
        btnSave.addEventListener('click', async () => {
            ...
            function detoxHTMLString(rawHtml) {
                try {
                    ...
                    const animatedElements = tempDiv.querySelectorAll('.gs-reveal, [data-animate], [data-gsap], .animate-on-scroll, .fade');
                    animatedElements.forEach(el => {
                        if (el.style) {
                            ...
                            // Scrub px heights usually injected by GSAP pinned sections
...
// If first match fails, fallback
if (!content.includes('detoxHTMLString')) {
   content = content.replace(
       /btnSave\.addEventListener\('click',\s*async\s*\(\)\s*=>\s*\{\s*\/\/\s*Helper to clean.*?const json = editor\.getProjectData\(\);/s,
       cleanerLogic
   );
}

fs.writeFileSync(file, content);
console.log('Final Editor script patched successfully');
