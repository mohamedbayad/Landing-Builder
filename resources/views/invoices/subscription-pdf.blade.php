...
<body>
    @php
        $issueDate = optional($subscription->created_at)->format('Y-m-d') ?: now()->format('Y-m-d');
        $customerName = $subscription->user?->name ?: 'Subscriber';
        $sellerName = config('app.name', 'LandingBuilder');
        $planName = $subscription->plan?->name ?: 'Subscription Plan';
        $billingCycle = ucfirst((string) ($subscription->billing_cycle ?: 'monthly'));
        $periodStart = optional($subscription->starts_at)->format('Y-m-d') ?: '-';
        $periodEnd = optional($subscription->ends_at)->format('Y-m-d') ?: '-';
        $status = strtolower((string) ($subscription->payment_status ?: $subscription->status ?: 'paid'));
        $statusClass = in_array($status, ['failed', 'canceled', 'cancelled'], true)
            ? 'failed'
            : (in_array($status, ['pending', 'trial'], true) ? 'pending' : '');
    @endphp
        ...
        <div class="content">
                ...
                <tr>
                    <td style="padding-right: 8px;">
                        <div class="card">
                            ...
                            <div class="muted">{{ $subscription->user?->email ?: '-' }}</div>
                            @if(!empty($subscription->user?->company_name))
                                <div class="muted">{{ $subscription->user->company_name }}</div>
                            @endif
                        </div>
                    </td>
            ...
            <table class="items">
                ...
                <tbody>
                    <tr>
                        <td>
                            <div class="value-strong">{{ $planName }} Plan</div>
                            <div class="muted">Cycle: {{ $billingCycle }}</div>
                            <div class="muted">Period: {{ $periodStart }} to {{ $periodEnd }}</div>
                        </td>
            ...
            <div class="footer-note">
                Thank you for your subscription. For billing support, contact your account manager.
            </div>
        </div>
    </div>
 </body>
