$file = "c:\Users\DELL\Desktop\web app\system\resources\views\editor.blade.php"
$content = [System.IO.File]::ReadAllText($file)

$startMarker = "    <!-- LEFT SIDEBAR (Elementor Style) -->"
$endMarker = "    <!-- RIGHT CANVAS AREA -->"
    ...
    <div class="flex h-full shrink-0 z-20 shadow-2xl relative">
        ...
        <aside id="editor-rail" class="w-16 flex flex-col bg-[#1e1e1e] border-r border-[#333] shrink-0 h-full py-4 items-center justify-between">
            ...
            <div class="flex flex-col gap-4 w-full items-center">
                ...
                <!-- Rail Tabs -->
                <button class="rail-tab active w-10 h-10 flex flex-col items-center justify-center rounded-xl text-indigo-400 bg-[#2e2e2e]" data-target="panel-blocks" title="Add Elements">
        ...
        <aside id="editor-panel" class="w-[280px] flex flex-col bg-[#252525] border-r border-[#333] shrink-0 h-full overflow-hidden transition-all duration-300">
            ...
            <div id="panel-blocks" class="context-panel flex flex-col h-full">
                <div class="px-4 py-4 border-b border-[#333] shrink-0">
                    <h2 class="text-xs font-bold uppercase tracking-wider text-gray-400">Add Elements</h2>
                </div>
                <!-- Search -->
                <div class="p-3 border-b border-[#333] shrink-0">
                    <div class="relative">
                        <iconify-icon icon="lucide:search" class="absolute left-3 top-2.5 text-gray-500" width="16"></iconify-icon>
                ...
                <!-- Blocks Container -->
                <div id="blocks-container" class="flex-1 overflow-y-auto p-3 flex flex-col gap-2 relative">
            ...
            <div id="panel-edit" class="context-panel hidden flex flex-col h-full bg-[#252525]">
                ...
                <div class="flex-1 overflow-y-auto relative p-0">
                    ...
                    <div id="edit-advanced" class="edit-pane hidden pb-10">
                        <div id="panel-advanced"></div> <!-- Targeted by advanced-editing-controls.js -->
                    </div>
                </div>
            </div>

            <!-- PANEL: LAYERS -->
            <div id="panel-layers" class="context-panel hidden flex flex-col h-full pt-4">
                <div class="px-4 pb-3 border-b border-[#333] shrink-0">
                    <h2 class="text-xs font-bold uppercase tracking-wider text-gray-400">Structure</h2>
                </div>
                <div id="panel-layers-inner" class="flex-1 overflow-y-auto relative">
                    <!-- GrapesJS Layers Inject Here -->
                </div>
            </div>

            <!-- PANEL: AI ASSISTANT -->
            <div id="panel-ai" class="context-panel hidden flex flex-col h-full pt-4 bg-[#252525]">
                <div class="px-4 pb-3 border-b border-[#333] shrink-0 flex items-center gap-2">
                    <iconify-icon icon="lucide:sparkles" width="16" class="text-amber-500"></iconify-icon>
                    <h2 class="text-xs font-bold uppercase tracking-wider text-amber-500">AI Assistant</h2>
                </div>
                <div id="panel-ai-inner" class="flex-1 overflow-y-auto relative p-4">
                    <!-- AI Plugin Panel will mount here -->
                    <p class="text-xs text-gray-500 italic text-center mt-10">Select a text or section block to see AI suggestions.</p>
                </div>
            </div>

            <!-- PANEL: PAGE SETTINGS -->
            <div id="panel-settings" class="context-panel hidden flex flex-col h-full pt-4">
                <div class="px-4 pb-3 border-b border-[#333] shrink-0">
                    <h2 class="text-xs font-bold uppercase tracking-wider text-gray-400">Global Settings</h2>
                </div>
                <div class="p-4 flex flex-col gap-4">
                    <div class="p-4 bg-[#1a1a1a] border border-[#333] rounded-lg flex flex-col items-center justify-center text-center">
                        ...
                        <button class="mt-4 px-4 py-2 bg-[#333] hover:bg-[#444] text-white rounded text-xs font-semibold w-full">Open Settings</button>
                    </div>
                </div>
            </div>
...
$newContent = $content.Substring(0, $startIdx) + $newSidebar + $content.Substring($endIdx)
